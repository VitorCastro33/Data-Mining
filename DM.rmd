...
lm <- performanceEstimation(
  ...
  EstimationTask(metrics="mae",method=CV(nReps=2,nFolds=3)))
...
##K-Means Method
The k-means algorithm defines the centroid of a cluster as the mean value of the points within the cluster then iteratively improves the within-cluster variation. For each cluster, it computes the new mean using the objects assigned to the cluster in
the previous iteration[@Han:2011:DMC:1972541].

We used the following script to find the best k (from 2 to 10) value for clustering the data using kmeans:

```{r k_means, echo=FALSE}
set.seed(1234)
d = dist(train_data[,-81])
avgS = c()
for(k in 2:10){
  cl = kmeans(train_data[,-81],centers=k,iter.max=400)
...
```{r k_means_sil, echo=FALSE, comment=""}
...
avgS = c()
for(n in 2:10){
  cl = pam(train_data[,-81],k=n)
  s = silhouette(cl$clustering,d)
  avgS = c(avgS,mean(s[,3]))
}
```
...
```{r clara_sil, echo=FALSE, comment=""}
res = data.frame(nClusters=2:10,SilhCo=avgS)
res[order(res$SilhCo,decreasing=TRUE),]
```
...
```{r hi_sil, echo=FALSE, comment=""}
res = data.frame(nClusters=2:10,SilhCo=avgS)
res[order(res$SilhCo,decreasing=TRUE),]
```

As we can see, the best silhouette coefficient result was using kmeans with 2 centers.
...
```{r kplot, echo=FALSE, fig.height=7.3}
k2 = kmeans(train_data[,-81], centers=2, iter.max=300)
s = silhouette(k2$cluster,dist(train_data[,-81]))
plot(s)
```
